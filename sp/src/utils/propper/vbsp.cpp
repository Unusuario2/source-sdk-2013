...
extern float		g_maxLightmapDimension; //REMOVE
...
qboolean	leaktest; //REMOVE
qboolean	verboseentities; //REMOVE
qboolean	dumpcollide = false; //REMOVE 
...
void SplitSubdividedFaces( node_t *headnode ); // garymcthack //REMOVE
...
int OutputDisp(mapdispinfo_t* pMapDisp, smd_triangle_t *tri, smd_point_t *vertices, int num_tris, brush_texture_t* pTexture, bool disp_nowarp, const char* qc_cdmaterials)
{
	...
	//DispMapToCoreDispInfo( pMapDisp, &coreDispInfo, 0, 0 ); //REMOVE
...
//		CmdLib_FPrintf(QCFile, "$origin %f %f %f\r\n", origin.y, origin.x, origin.z );
	...
	if (dophysmodel)
	{
		...
		{ //Use phys model here also because it might differ from ref.
			CmdLib_FPrintf(QCFile, "$collisionmodel \"%s_phys\"  {\r\n", basename);
		}
//VERSIONSHIT
...
int RunPROPPER( int argc, char **argv )
{
	...
	//LoadSurfaceProperties(); //FIMXEEEEEEEEEEEEEE
	...
	LoadMapFile (name);
	//WorldVertexTransitionFixup(); //REMOVE

	//SetModelNumbers(); //REMOVE
	//ProcessModels (); //REMOVE
	...
	for (int i = 0; i < num_models; i++)
	{
		...
		if (objExport)
			V_snprintf(&SMDfilename[0], 1024, "%s\\%s_ref.obj", sourcefolder, m->qc_modelname);
		else
			V_snprintf(&SMDfilename[0], 1024, "%s\\%s_ref.smd", sourcefolder, m->qc_modelname);
	...
	//			Msg("Studiomdl output proceeds below...\n\n----------------------\n");
			...
			ZeroMemory( &pi, sizeof(pi) );
				...
				*/
			if (!CreateProcess(NULL,studioCommand,NULL,NULL,false,0x00000000, NULL,NULL, &si,&pi ))
